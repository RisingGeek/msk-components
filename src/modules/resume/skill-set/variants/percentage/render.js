...
const Basic = ({ theme, headlineText, skills }) => {
  ...
  const skillItem = (skill, id) => {
    return (
      <div key={id} style={styles.flexBox}>
        <div style={styles.skill}>{skill.value.title.value}</div>
        <div style={styles.bar}>
          <div
            style={{
              ...styles.barLen,
              width: skill.value.percentage.value + '%'
            }}
          ></div>
          {skill.value.percentage.value < 80 &&
            skill.value.percentage.value + '%'}
        </div>
  ...
  return (
    <div style={styles.root}>
      ...
      <hr />
      <br />
      <div>{skills.value.map((skill, id) => skillItem(skill, id))}</div>
    </div>
  );
};
